...
- 手機版導覽預設收合為品牌與選單按鈕，減少頁首占用；支援展開、選頁後收合與 Escape 返回按鈕焦點。

- 捉迷藏小貓限制在內容容器的可見範圍，支援上下左右探頭並優先換方向；重新設計貓掌標記、指向尾巴與文字排版的對話泡泡，窄畫面也保持在內容區內。

- 資料庫備份改為所有類型合計保留最新一筆，僅在新備份成功後清理舊資料庫與 manifest；刪除失敗保留追蹤紀錄，於下次備份重試。
- 新增資料來源切換至 WSL 時自動偵測已安裝的 Linux 環境；只有一個環境時自動填入，並支援清單選擇、手動輸入與重新偵測。

### 修正

- 修正工作填寫頁在中等視窗寬度下，表單與 Markdown 編輯器溢出並遮擋右側工作紀錄；工作台與表單依可用寬度自動改排。
...
- 工作摘要的「瀏覽日期」週檢視現在顯示月／日與星期，例如 `9/7(一)`，方便在單列日期卡中辨識日期。
- Azure DevOps 資料回補顯示各來源名稱、類型、已讀取／總檔案數與處理階段；回補期間最長可達 365 天，且切換功能、重新整理或關閉分頁前會提醒確認，避免誤中斷。
- Azure DevOps 長期間回補新增專案／Repo 查詢、PR 詳細資料與工作項目異動紀錄，以及內容和留言的已處理／總數進度。
- AI 日／週摘要與來源回補完成或失敗時，會以分頁標題提示處理結果；可在通知設定或執行中的提示啟用桌面通知，通知不包含工作內容。
- 畫面操作失敗會統一寫入本機 Error log，方便診斷草稿編輯、來源狀態更新、設定匯入、同步與排程問題。

### 修正

- 修正 Antigravity CLI 將固定的 JSON 格式錯誤一律視為尚未寫完並持續重報警告的問題；未變動的失敗檔案不再反覆讀取，檔案變動後仍會重試並保留有效對話。
- 修正回補清單隱藏尚未就緒來源的問題。啟用來源與歷史回補會自動驗證；驗證失敗會顯示原因，沒有可回補來源時不再顯示成功。
- 修正 Azure DevOps Work Item 在 CLI 成功但輸出空白、或診斷文字混入標準輸出時被誤判為 JSON 錯誤的問題。
- Azure DevOps 驗證或同步失敗時，本機日誌會保留各設定的失敗明細；無法解析的回應會額外記錄命令階段、結束碼與受限長度的診斷輸出，方便排查 CLI 或 extension 問題。
