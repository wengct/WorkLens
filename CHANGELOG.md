...
- Azure DevOps 回補新增專案／Repo 查詢階段、PR 詳細資料及工作項目異動紀錄、內容與留言的已處理／總數，方便追蹤長期間回補進展。

- 年度 AI 草稿使用 ask-bridge 且資料較大時，先分段抽取完整年度證據，再依原始年度 Prompt 執行一次最終整合，不會直接串接各段結果；等待視窗會顯示目前階段、段落進度、摘要壓縮及附件重試狀態。附件 MCP 工作階段重設時會自動重試一次，任一階段失敗或中斷皆保留原草稿。送出前仍記錄資料、Prompt 與合計字元數的資訊日誌（不含內容），完成通知亦顯示字元數，並明確提供回覆 ID 及指出無效 GUID 欄位。
- 畫面錯誤通知統一記錄 Error log，補齊草稿編輯、年度回顧、來源狀態更新、設定匯入、同步及排程失敗的日誌。

- 年度 AI 草稿在準備資料及機敏資訊檢查期間顯示載入動畫與等待說明，大量資料準備移出 UI 執行環境，完成後再顯示遮蔽確認。

- 修正既有年度回顧的日期區間顯示，統一使用 yyyy/MM/dd，避免格式字串直接出現在畫面上。

- 修正既有年度回顧的日期區間顯示，統一使用 yyyy/MM/dd，避免格式字串直接出現在畫面上。

- 來源資料回補顯示各來源名稱、類型、已讀取／總檔案數與處理階段，並保留完成、失敗及停止狀態；非檔案來源以階段文字呈現。

- 修正回補清單隱藏尚未就緒來源的問題；啟用來源與歷史回補會自動驗證，驗證失敗會顯示原因，無可回補來源時不再顯示成功。

- 新增 Google Antigravity CLI 本機對話來源，支援 Windows、WSL、macOS、自訂資料目錄、增量收集與歷史回補；今日工作台及報告可展開使用者與 AI 對話，並保留部分資料重試及既有 AI 分享限制。支援格式為 transcript_full.jsonl，實際 CLI 逐字稿及 WSL／macOS 收集仍待實機驗證。
- 形象網站的支援來源與工作流程介紹新增 Google Antigravity CLI。

- 新增 Google Antigravity CLI 本機對話來源，支援 Windows、WSL、macOS、自訂資料目錄、增量收集與歷史回補；今日工作台及報告可展開使用者與 AI 對話，並保留部分資料重試及既有 AI 分享限制。支援格式為 transcript_full.jsonl，實際 CLI 逐字稿及 WSL／macOS 收集仍待實機驗證。
- 形象網站的支援來源與工作流程介紹新增 Google Antigravity CLI。

- AI 日／週摘要、年度回顧 AI 草稿與來源回補新增分頁標題處理結果提醒；可在通知設定或執行中的提示開啟桌面通知，並提供權限狀態、關閉與測試通知。通知只包含一般處理結果，不含工作內容。
- 來源資料回補中切換功能會先確認是否離開；重新整理或關閉分頁時也會顯示瀏覽器離開提醒，避免誤操作中斷回補。
- 修正 Azure DevOps Work Item 查無資料時，CLI 成功但輸出空白被誤判為 JSON 錯誤，導致來源驗證或同步出現失敗訊息的問題。

- 年度回顧的明細、編輯與機敏預覽改用彈跳視窗，統一面板間距，補齊載入及儲存狀態，並保留補充成果操作前尚未儲存的草稿。

- 修正 Azure DevOps CLI 將診斷文字混入標準輸出時，Work Item 同步誤判回應不是 JSON 的問題。
- 驗證 Azure DevOps 等來源失敗時，本機日誌現在會保留各設定的具體失敗明細。
- Azure DevOps CLI 回傳無法解析的 JSON 時，本機日誌現在會保留原始 stdout，若 stdout 空白則改保留 stderr（最多 16 KiB），以便診斷 CLI 或 extension 回應。
- Azure DevOps Work Item 查詢回傳非 JSON 時，本機 Error log 會額外記錄命令階段、結束碼及 stdout／stderr 長度。
- 來源資料回補的單次期間由最多 90 天擴大為最多 365 天，仍不會改變正常排程 checkpoint。
- 年度回顧現在可直接從符合 AI 納入設定的工作紀錄與來源活動產生草稿；人工成果改為選填，同步資料需明確勾選才會納入。
- 新增「年度回顧」：可選擇最多一年的期間，檢視每月資料涵蓋，提供人工補充及可編輯的績效考核草稿。

## [1.10.2] - 2026-09-12

- 機敏資訊防護頁的自訂敏感詞與掃描排除關鍵字改為桌面雙欄並排，窄螢幕維持單欄。
- 縮減左側選單項目與區段間距，並針對較矮的筆電視窗進一步壓縮留白。
- 工作台的四張狀態卡移到右欄工作紀錄上方，以 2×2 排列。
- 工作摘要與日期瀏覽在桌面第一屏並排顯示，週檢視改為一天一列；窄螢幕優先顯示摘要，歷程篩選放在來源明細上方，主要操作與匯出操作分列。
- 工作填寫現在會優先顯示輸入區；工作摘要會先提供摘要工作區與固定的整理方式、AI 整理及儲存工具列，減少進入頁面後捲動操作。
- 工作摘要的「整理方式」與「其他操作」改為固定顯示，不再需要反覆展開。
- 更新 GitHub Pages 官網的工作流程介紹、章節導覽與摘要工作區畫面，並補齊 README 介面預覽。
- 修正同步匯入的來源佐證在 SQLite 產生每日或每週基本摘要時，因日期範圍查詢無法轉譯而使 AI 整理失敗的問題。
- AI 報告回覆不再驗證或要求 `totalHours`，避免 AI 可見工作資料範圍與報告總工時不同時阻擋摘要產生；報告與工作紀錄 ID 驗證仍會保留。
...
- 工作紀錄與來源佐證會匯入為唯讀遠端副本，歷程、摘要與 CSV 可讀取完整跨裝置資料；AI 納入仍維持本機控制。
- 已驗證的同步批次會以內容雜湊建立本機索引，後續掃描直接略過已處理檔案，避免重複讀取與解析。

## [1.9.0] - 2026-09-10

- 修正 Azure DevOps Work Item 的舊 revision 在後續異動時被誤收為當日活動，以及最新 revision 因占位日期而漏收的問題；活動日期現在以實際 `ChangedDate` 為準。
- 工作台現在可逐筆刪除自動來源活動；工作台與工作摘要的每日檢視也可一次刪除指定日期的全部自動收集紀錄。手動參考資料、其他日期、來源設定與原始來源不受影響，相關每日與每週摘要會標示為需要重產。

## [1.8.1] - 2026-09-10

- 資料來源驗證失敗或發生未預期錯誤時，現在會寫入本機 log，方便排查環境與路徑問題。

## [1.8.0] - 2026-09-10

- 修正 macOS 背景啟動時無法找到由 nvm 管理的預設 Node.js；AI 設定頁也會顯示 ask-bridge 環境診斷細節，方便確認缺少的執行條件。
