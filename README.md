...
WorkLens 每次準備把工作資料交給 AI 前，都會在本機掃描最終工作資料與有效 Prompt。發行包目前隨附固定的官方 `leak-hunter v0.5.4` 執行檔，並在建置時以 SHA-256 驗證；使用者不需要另外安裝 `cargo`／`npm` 套件，也不會從系統 PATH 使用其他版本。更新 WorkLens 時會一併更新掃描器，版本檢查失敗會停止 AI 傳送。WorkLens 也會補充 Email 與自訂敏感詞比對。若掃描器誤判特定完整值，可在「機敏資訊防護」設定掃描排除關鍵字；排除項目會原樣傳送，且不會影響 Email 與自訂敏感詞遮蔽。
