...
![設定移轉](screenshot/10.設定移轉.png)

可從「設定移轉」選擇匯出 AI 設定、Prompt 範本、資料來源及選用的機敏規則，再匯入另一台電腦。API Key、工作紀錄、摘要與來源收集狀態不會包含在設定檔；匯入的資料來源會保持停用，請在新電腦重新驗證路徑與環境後再啟用。若選擇 Git 作者 Email、自訂敏感詞或掃描排除關鍵字，匯出檔可能包含個人資料或憑證值，請視為機密檔案保管。
...
![機敏資訊防護](screenshot/07.機敏資訊防護.png)
...
WorkLens 每次準備把工作資料交給 AI 前，都會在本機掃描最終工作資料與有效 Prompt。發行包目前隨附固定的官方 `leak-hunter v0.5.4` 執行檔，並在建置時以 SHA-256 驗證；使用者不需要另外安裝 `cargo`／`npm` 套件，也不會從系統 PATH 使用其他版本。更新 WorkLens 時會一併更新掃描器，版本檢查失敗會停止 AI 傳送。WorkLens 也會補充 Email 與自訂敏感詞比對。若掃描器或 Email 規則誤判特定完整值，可在「機敏資訊防護」設定掃描排除關鍵字；排除項目會原樣傳送，自訂敏感詞仍會遮蔽。

命中時只修改這一次 AI 請求的記憶體副本，使用 `[已遮蔽：機敏憑證]`、`[已遮蔽：個人資料]` 或 `[已遮蔽：自訂敏感詞]` 取代命中內容。Git、Azure DevOps、人工紀錄與 Prompt 範本的原始資料不會回寫。手動整理只有發生遮蔽時才顯示預覽，使用者可選擇「遮蔽後送出」或「取消」；沒有需要遮蔽的內容會直接送出。排程則自動送出已通過第二次掃描的副本。掃描器缺失、逾時、結果無法解析、定位不一致或遮蔽後仍有非排除項目命中時，WorkLens 會停止這次 AI 傳送並保留既有報告。
