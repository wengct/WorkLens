...
- 不要只依賴 `.panel + .panel` 控制垂直間距；面板被 grid、條件區塊或其他容器包住後，就不再是相鄰兄弟元素。
- 同一頁有多個固定或條件顯示的面板時，優先以頁面層級的 stack 容器和 `gap` 統一管理間距，並重設子面板可能繼承的 `margin-top`，避免間距消失或重複累加。
- 調整頁面結構後，逐一檢查條件面板顯示與隱藏的組合，以及窄螢幕排列；版面回歸測試應鎖定容器與間距規則，而不是只檢查單一面板存在。
