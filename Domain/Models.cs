...
public static class AiPromptDefaults
{
    public const string GeneralReportPrompt = """
        請將資料整理成清楚、可直接交付的工作回報，並依據工作內容自動歸類，自動預估工時，以 0.5 為單位，按以下固定分類拆分章節：專案管理、UIUX相關、需求評估、功能開發、功能測試、BUG處理、文件相關、客服、其他。分類名稱、文字與順序不可更動；只建立有內容的章節。同一筆工作若涉及多個分類，歸入最主要的分類，避免重複；判斷有多個專案應分開。保留具體成果與處理過程；以繁體中文撰寫，內容精簡但不可遺漏重要脈絡。
